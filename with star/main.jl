...
const shellThicknessFactor = 1.0032  # Thickness of each consecutive shell grows exponentially by this rate factor 1.0032
const extend_factor = 4  # Maximum halo radius at initialization is set as R_vir * extend_factor. 4 is recommended

############### Star Profile #####################
profile_type = 4		#0 = turn off; 1 = king(1962); 2 = plummer; 3 = Hernquist; 4 = King(1966)
Mass_star = 1e-4		#in 1e10 solar mass. Meaning: (king1962) Mass inside tidal radius; (plummer) total mass of cluster; (Hernquist) Total Mass
King_c = 400			#(King1962) Core radius, in kpc
King_t = 700			#(King1962) Tidal radius. in kpc
Plummer_a = 1			#(Plummer) Plummer radius. in kpc
Hern_a = 1			#(Hernquist) Hernquist radius. in kpc
King66_t = 10			#(King1966) Tidal radius. in kpc. Determines Potential function and escape velocity
King66_disper = 20		#(King1966) velocity dispersion, in km/s
King66_k = 1			#(king1966) Normalization constant in distribution function, unitless
...
#Star Profile
if profile_type==1
Star_param[1]=Mass_star
Star_param[2]=King_c
Star_param[3]=King_t
elseif profile_type == 2
Star_param[1]=Mass_star
Star_param[2]=Plummer_a
Star_param[3]=0
...
Star_param=[King66_k,King66_t,King66_disper]
else
Star_param=[0,0,0]
end
####################################################################################################
...
function dmOnly()
    ...
    # Master folder
    folderName = "dmOnly_test"
    if !isdir(folderName)
        mkdir(folderName)
    end
    
    # Parameter-specific subfolder
    folderName = folderName * "/" * string(M_vir) * "_" * string(v_k) * "_" * string(tau) * "_" * string(numOfShells) * "_" * string(numOfSteps)  * "_" * string(profile_type) * string(Star_param)
    if !isdir(folderName)
        mkdir(folderName)
    end
    ...
    # Initilize Star Shells
    Sshells_mass = Star_initialization(Tshells_radii,profile_type,Star_param)


#iteration between star and DM
    Tshells_mass_iteration = Tshells_mass
    Tshells_mass_iteration_star = Tshells_mass_iteration + Sshells_mass

    Tshells_mass = adiabaticExpansion(Tshells_radii, Tshells_mass, Tshells_mass_iteration, Tshells_mass_iteration_star)
    Tshells_mass_iteration = Tshells_mass
    Tshells_mass_iteration_star = Tshells_mass_iteration + Sshells_mass
    Sshells_mass = adiabaticExpansion(Tshells_radii, Sshells_mass, Tshells_mass_iteration, Tshells_mass_iteration_star)
