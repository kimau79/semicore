...
function Star_initialization(Shells_radii,type,Star_params)
	Starshell_mass = zeros(size(Shells_radii,1))
	if type == 1
		x_t = (Star_params[3]/Star_params[2])^2
		k = Star_params[1]/(pi*Star_params[2]^2*(ln(1+x_t)-4*(sqrt(1+x_t)-1)/sqrt(1+x_t)+x_t/(1+x_t)))
		x = (Shells_radii[1,1]/Star_params[2])^2
		upper=k*pi*Star_params[2]^2*(log(1+x)-4*(sqrt(1+x)-1)/sqrt(1+x_t)+x/(1+x_t))
		for i=1:size(Shells_radii,1)
			lower=upper
			x = (Shells_radii[i,2]/Star_params[2])^2
			upper=k*pi*Star_params[2]^2*(log(1+x)-4*(sqrt(1+x)-1)/sqrt(1+x_t)+x/(1+x_t))
			Starshell_mass[i]=upper-lower
		end
	elseif type == 2
		for i=1:size(Shells_radii,1)
			lower=Star_params[1]*Shells_radii[i,1]^3/sqrt(Shells_radii[i,1]^2+Star_params[2]^2)^3
			upper=Star_params[1]*Shells_radii[i,2]^3/sqrt(Shells_radii[i,2]^2+Star_params[2]^2)^3
			Starshell_mass[i]=upper-lower
		end		
	elseif type == 3
		for i=1:size(Shells_radii,1)
			lower=Star_params[1]*Shells_radii[i,1]^2/(Shells_radii[i,1]+Star_params[2])^2
			upper=Star_params[1]*Shells_radii[i,2]^2/(Shells_radii[i,2]+Star_params[2])^2
			Starshell_mass[i]=upper-lower
		end
	elseif type == 4
		esti = 1e-7
		j=sqrt(1/2)/Star_params[3]
#		f1(r,v) = (exp(-j^2*v^2-2*j^2*V(r))-1)*4*pi*v^2
		tidal_V = -4*pi*G*rho_0*R_s^3/Star_params[2]*log(1+Star_params[2]/R_s)
		V(r) = -4*pi*G*rho_0*R_s^3*(1/R_s+r/(2*R_s^2)+r^2/(3*R_s^3)+r^3/(4*R_s^4)+r^4/(5*R_s^5))-tidal_V
#		Rho(r) = quadgk(v->f1(r,v),0,sqrt(-2*V(r)))[1]
#		f2(r) = Rho(r) * 4 * pi*r^2
#		M_star(lower,r) = DIY(f2,lower,r)
		W1(r)=-2*j^2*V(r)
		Rho2(r)=(4*pi*Star_param[1]/(3*j^3))*exp(W1(r))*(3/4*sqrt(pi)*erf(sqrt(W1(r)))-exp(-W1(r))*sqrt(W1(r))/2*(3+2*W1(r)))*4*pi*r^2
		Ass = quadgk(Rho2,0,esti)[1]
		V(r) = -4*pi*G*rho_0*R_s^3/r*log(1+r/R_s)-tidal_V
#		C=zeros(size(Shells_radii,1),2)
		for i=1:size(Shells_radii,1)
#			C[i,2]=quadgk(Rho2,Shells_radii[i,1],Shells_radii[i,2])[1]
			if Shells_radii[i,1]<esti
#			Starshell_mass[i]=(M_star(esti,Shells_radii[i,2])+Ass)*Star_param[1]
			Starshell_mass[i]=(quadgk(Rho2,esti,Shells_radii[i,2])[1]+Ass)*Star_param[1]*h/1e10
			elseif Shells_radii[i,1]>Star_params[2]
			Starshell_mass[i]=0
			elseif (Shells_radii[i,1]<Star_params[2]) && (Shells_radii[i,2]>Star_params[2])
			Starshell_mass[i]=(quadgk(Rho2,Shells_radii[i,1],Star_params[2])[1])*Star_param[1]*h/1e10
			else
#			Starshell_mass[i]=M_star(Shells_radii[i,1],Shells_radii[i,2])*Star_param[1]
			Starshell_mass[i]=(quadgk(Rho2,Shells_radii[i,1],Shells_radii[i,2])[1])*Star_param[1]*h/1e10
			end
#			C[i,1]=Starshell_mass[i]
		end
#		outputC_file= "dmOnly/star_density.txt"
#		writedlm(outputC_file,C)
	end
	return Starshell_mass
end
