...
function GPE_extrapolate(r, shells_radii, shells_GPE)
...
function GPE_extrapolate2(r, shells_radii, shells_GPE)	#derivative of GPE extrapolate
#use power law GPE(r) = -Ar^n to extrapolate, for some A, n to be determined
if r>shells_radii[end,3] #the only case that this function should be used
ref_GPE1 = shells_GPE[end-1,1]
ref_GPE2 = shells_GPE[end,1]
...
function energyEquation(r, L, totalE_afterDecay, Tshells_radii, Tshells_GPE)
    ...
    elseif r > Tshells_radii[end,3]
        return  GPE_extrapolate(r,Tshells_radii, Tshells_GPE)  + (L / r) ^ 2 / 2 - totalE_afterDecay  
...
function U_eff_gfunction(x,r_max,r_min, L, Tshells_radii, Tshells_GPE)
    ...
    elseif r > Tshells_radii[end, 3]  # r big
        ...
        return GPE_extrapolate(r,Tshells_radii, Tshells_GPE) + (L / r) ^ 2 / 2 
    else  # r in between; value by interpolation
        radiusIndex = -1  # Just for the definition
        for i in 2:size(Tshells_radii, 1)
            if r <= Tshells_radii[i, 3]
...
function dU_eff(r, L, Tshells_radii, Tshells_enclosedMass, Tshells_GPE)    
    ...
    elseif r > Tshells_radii[end, 3]  # r big
        return GPE_extrapolate2(r,Tshells_radii, Tshells_GPE)  - (L / r) ^ 2 / r
#the function GPE_extrapolate is reused for Mass extrapolation since the concept is the same as GPE extrapolation
    else  # r in between; value by interpolation
        radiusIndex = -1  # Just for the definition
        for i in 2:size(Tshells_radii, 1)
            if r <= Tshells_radii[i, 3]
...
output=zeros(size(slope,1),3)
for i=1:size(slope,1)
output[i,1]=x_node[i+1,1]
output[i,2]=slope[i,1]
output[i,3]=constant_c[i,1]
end
#writedlm("dmOnly/0.517_40_3_3353_1/SL/" * string(t_i-1) * "/" * string(r_min)*".txt",output)
...
#writedlm("dmOnly/0.517_40_3_3353_1/Poly/" * string(t_i-1) * "/" * string(r_min)*".txt",soln)
...
#writedlm("dmOnly/0.517_40_3_3353_1/WeightFactor/" * string(t_i-1) * "/" * string(shells_radii[i,2])*".txt",weightFactor)
